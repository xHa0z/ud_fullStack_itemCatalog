...
from database_setup import Department, Base, Stock
...
# Stock for Auto
dept1 = Department(name="Auto")

session.add(dept1)
session.commit()

stock1 = Stock(name="Air Filter", brand="ACF",
                  num_in_stock=60, department=dept1)
...
stock2 = Stock(name="Wiper Fluid", brand="OXG",
               num_in_stock=35, department=dept1)

session.add(stock2)
session.commit()

stock3 = Stock(name="Spark Plug", brand="CSD",
               num_in_stock=120, department=dept1)

session.add(stock3)
session.commit()

stock4 = Stock(name="All Season Mat", brand="MTT",
               num_in_stock=77, department=dept1)
...
stock1 = Stock(name="CPU", brand="ITL",
                  num_in_stock=60, department=dept2)
...
stock2 = Stock(name="DLSR", brand="CNS",
               num_in_stock=12, department=dept2)

session.add(stock2)
session.commit()

stock3 = Stock(name="Projector", brand="ESP",
               num_in_stock=34, department=dept2)

session.add(stock3)
session.commit()

stock4 = Stock(name="Wireless Headset", brand="SNY",
               num_in_stock=67, department=dept2)

session.add(stock4)
session.commit()

stock5 = Stock(name="Tablet", brand="GLP",
               num_in_stock=23, department=dept2)
...
# Stock for Outdoors
dept3 = Department(name="Outdoors")

session.add(dept3)
session.commit()

stock1 = Stock(name="Tent", brand="STK",
                  num_in_stock=23, department=dept3)
...
stock2 = Stock(name="Bike", brand="SSZ",
               num_in_stock=7, department=dept3)

session.add(stock2)
session.commit()

stock3 = Stock(name="Backpack", brand="NFT",
               num_in_stock=38, department=dept3)
